...
contract CrossChainForwarderHarness is CrossChainForwarder {
  ...
  ) CrossChainForwarder(bridgeAdaptersToEnable, sendersToApprove, optimalBandwidthByChain) {}
  ...
  function _bridgeTransaction(
    ...
    if (_last_nonceTX_sent > _max_TXnonce) _max_TXnonce = _last_nonceTX_sent;

    if (_TXid_2_TXnonceP1[transactionId] == 0) {
      // this is the first time that this TX is sent.
  ...
  function get_envelope_from_encodedTX(
    bytes memory encodedTransaction
  ...
  function get_transaction_ID_from_encodedTX(
    bytes memory encodedTransaction
  ) external pure returns (bytes32) {
    return TransactionUtils.getId(encodedTransaction);
  }

  function get_envelopID_from_encodedTX(
    bytes memory encodedTransaction
  ) external pure returns (bytes32) {
    Transaction memory transaction = TransactionUtils.decode(encodedTransaction);
  ...
  function getForwarderBridgeAdaptersByChain_len(uint256 chainId) external view returns (uint) {
    return _bridgeAdaptersByChain[chainId].length;
  }

  function getForwarderBridgeAdaptersByChainAtPos_dest(
    uint256 chainId,
    uint256 pos
  ...
  function getForwarderBridgeAdaptersByChainAtPos_curr(
    uint256 chainId,
    uint256 pos
  ) external view returns (address) {
