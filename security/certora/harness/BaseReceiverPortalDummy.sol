...
contract BaseReceiverPortalDummy {
  ...
  mapping(address => mapping(uint256 => mapping(bytes => uint256)))
    internal _receive_cross_chain_message_counter;

  // Decides nondeterministically whteher to revert
  function receiveCrossChainMessage_reverts() internal view returns (bool) {
    return _receiveCrossChainMessage_reverts;
  }
